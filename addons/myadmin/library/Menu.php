...
class Menu
{
    ...
    public static function upgrade($name, $menu)
    {
        ...
        try {
            ...
            if ($ids) {
                ...
                $menus = isset($config['myadmin_menus']) ? $config['myadmin_menus'] : [];
                $where = ['id' => ['in', $ids]];
                if ($menus) {
    ...
    public static function refresh($name, $menu = [])
    {
        if (!$menu) {
            ...
            $menus = Db::name("myadmin_auth_rule")->where('id', 'in', $menuIds)->column('name');
        } else {
            // 刷新新的菜单缓存
        ...
        //刷新新的插件核心菜单缓存
        Service::config($name, ['myadmin_menus' => $menus]);
    }
     ...
     * @param mixed $parent 父类的name或pid
     */
    public static function myadminrCeate($menu = [], $parent = 0)
    {
        self::myadminMenuUpdate($menu, $parent);
    }
     ...
     * @throws Exception
     */
    private static function myadminMenuUpdate($newMenu, $parent = 0)
    {
        if (!is_numeric($parent)) {
            $parentRule = AuthRule::getByName($parent);
            $pid = $parentRule ? $parentRule['id'] : 0;
        ...
        foreach ($newMenu as $k => $v) {
            ...
            $data['pid'] = $pid;
            $data['status'] = 'normal';
            $oldMenu = AuthRule::field('id')->where('name', $data['name'])->find();
            if ($oldMenu) {
                $menu = $oldMenu;
                AuthRule::update($data, ['id' => $menu['id']]);
            } else {
            ...
            if ($hasChild) {
                self::myadminMenuUpdate($v['sublist'], $menu['id']);
            }
        }
    }
}
