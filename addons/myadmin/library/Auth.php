...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        // 检查企业信息
        if ($company = Company::where('id', $admin['company_id'])->find()) {
            // 赋值企业ID
            ...
            if (COMPANY_ID != $company['id']) {
                $this->setError('非法登录企业');
                return false;
            }
            //检测审核中状态
            if (in_array($company['status'], ['created'])) {
                $this->setError('该企业审核中，请联系管理员！');
                return false;
            }
            //检测禁用状态
            if (in_array($company['status'], ['expired'])) {
                $this->setError('该企业已被禁用，请联系管理员！');
                return false;
            }
            //检查有效期状态
            ...
            if (isset($info['termvalidity']) && $info['termvalidity'] == 'vip') {
                ...
                if (!$vip = get_addon_info('vip')) {
                    $this->setError('请先安装FastAdmin官方VIP插件！');
                    return false;
                }
                if (!$vip['state']) {
                    $this->setError('请先启用FastAdmin官方VIP插件！');
                }
                if (!$vipInfo = \addons\vip\library\Service::getVipInfo($company['id'])) {
                    $this->setError('该企业已过期，请续费！');
                    return false;
                }
            }
        } else {
            $this->setError('Identifier is incorrect');
            return false;
        }
        // 验证登录限制
        ...
        $admin->save();
        Session::set("myadmin", $admin->toArray());
    ...
    {
        $keeplogin = Cookie::get("mykeeplogin");
        if (!$keeplogin) {
            return false;
        }
    ...
    protected function keeplogin($keeptime = 0)
    {
        if ($keeptime) {
            ...
            Cookie::set("mykeeplogin", implode('|', $data), 86400 * 7);
            return true;
        }
        return false;
    }
    ...
    {
        $uid = $uid ? $uid : $this->id;
        $rulelist = $this->getPlayerRuleList($uid);
        if (!in_array($name, $rulelist)) {
            return false;
        }
     ...
     * 角色权限组
     */
    public function getPlayerRuleList($uid = null)
    {
        static $_playerrulelist = [];
        if ($_playerrulelist) {
            return $_playerrulelist;
        }
        $rule_id = $this->getPlayerRuleIds();
        $rulelist = Db::name($this->config['auth_rule'])->where('id', 'in', $rule_id)->column('name');
        $_playerrulelist = $rulelist;
        return $_playerrulelist;
    }
     ...
     * 角色ID组
     */
    public function getPlayerRuleIds($uid = null)
    {
        static $_playerruleid = [];
        if ($_playerruleid) {
            return $_playerruleid;
        }
        $_playerruleid = Company::getRules($this->company_id);
        return $_playerruleid;
    }
    ...
    public function getPlayer()
    {
        ...
        $_player = Company::getPlayer($this->company_id);
        return $_player;
    }
     ...
     * 角色标签检测
     */
    public function getPlayerLabel($label = null)
    {
        $player_list = $this->getPlayer();
        $player = [];
        foreach ($player_list as $key => $ov) {
            if ($ov['status']) {
                $player[] = $key;
            }
        }
        if ($label) {
            return isset($player[$label]) ? true : false;
        }
        return $player;
    }
     ...
     * 当前企业
     */
    public function getCompanyInfo($uid = null)
    {
        static $_company = [];
        if ($_company) {
            return $_company;
        }
        $company = Company::get($this->company_id);
        $_company = $company ? $company->toArray() : [];
        return $_company;
    }
    ...
    public function getChildrenGroupIds($withself = false)
    {
        ...
        // 取出所有分组
        $groupList = \addons\myadmin\model\AuthGroup::where(['status' => 'normal'])->where('company_id', 'in', [0, $this->company_id])->select();
        $objList = [];
        foreach ($groups as $k => $v) {
            if ($v['rules'] === '*') {
                $objList = $groupList;
    ...
    public function getChildrenAdminIds($withself = false)
    {
        ...
        if (!$this->isSuperAdmin()) {
            ...
            $authGroupList = \addons\myadmin\model\AuthGroupAccess::field('uid,group_id')->where('group_id', 'in', $groupIds)->select();
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        $playerRuleIds = $this->getPlayerRuleIds();
        $selected = $referer = [];
        $refererUrl = Session::get('referer');
        // 必须将结果集转换为数组
        $ruleList = collection(\addons\myadmin\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache("__my_menu__")
            ->select())->toArray();
        $indexRuleList = \addons\myadmin\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 0)
            ->where('name', 'like', '%/index')
            ->column('name,pid');
        $pidArr = array_unique(array_filter(array_column($ruleList, 'pid')));
        foreach ($ruleList as $k => &$v) {
            if (!in_array($v['id'], $playerRuleIds)) {
                unset($ruleList[$k]);
                continue;
            }
