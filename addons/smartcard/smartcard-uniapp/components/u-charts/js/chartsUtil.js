	...
	* target-this(全局对象this)
	...
	showCharts(canvasId,data,target){
		return new uCharts({
			...
			fontSize: data.fontSize||12,//全局默认字体大小，单位为px，默认13px，高分屏不必乘像素比，自动计算 
			background: data.background||'#4a85b2',//canvas背景颜色（如果页面背景颜色不是白色请设置为页面的背景颜色，默认#ffffff）
			...
			animation: data.animation||true,//是否动画展示，默认为 true
			...
			//series:data.color||'#ff0000',//|例如#7cb5ec不传入则使用系统默认配色方案
		   ...
		   title: data.title||{//适用于`ring`、`arcbar`、`gauge`
			   ...
			   color: data.titlecolor||((data.type=='arcbar'||data.type=='gauge')?data.series[0].color:'#ffffff'),//标题颜色（可选）
		   ...
		   subtitle: data.subtitle||{//适用于`ring`、`arcbar`、`gauge`
			   ...
			   color: data.subtitlecolor||'#fff',//副标题颜色（可选）
		   },
			...
			yAxis: data.yAxis||{
				...
				splitNumber:data.yAxissplitNumber||7,//Y轴网格数量
				gridType:data.yAxisgridType||'dash',//Y轴网格线型 'solid'为实线、'dash'为虚线
				dashLength:data.yAxisdashLength||4,//Y轴网格为虚线时，单段虚线长度
				gridColor:data.yAxisgridColor||'#fff',//Y轴网格颜色 例如#7cb5ec
				fontColor:data.yAxisfontColor||'#fff',//Y轴数据点颜色 例如#7cb5ec
				titleFontColor:data.yAxistitleFontColor||'#fff',//Y轴title颜色 例如#7cb5ec
			},
			extra: data.extra||{
				...
				gauge:data.gauge||{//仪表盘相关配置
					...
					labelColor:data.labelColor||'#ffffff',//默认#666666|仪表盘刻度尺标签文字颜色",
	...
	"opts.extra.tooltip.activeBgOpacity |Number |默认0.08 | 仅柱状图类适用，当前点击柱状图的背景颜色透明度 "
				...
				legendTextColor:data.legendTextColor||'#fff',//'#cccccc',//图例文案颜色 例如#7cb5ec`后期将变更为opts.legend.textColor迁移到基础配置里
