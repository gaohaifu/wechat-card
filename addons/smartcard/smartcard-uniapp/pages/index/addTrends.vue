	...
	<view>
      ...
      <view class="Inside_fast Inside_fast_padding">
				<view class="infoItem flex_layout">
					<view class="left_title">动态名称</view>
					<view class="right_info flex_layout">
						<input type="text" v-model="title" placeholder="请填写动态名称" value="" />
					</view>
				</view>
				<view class="infoItem infoItem_textarea">
					<view class="left_title">动态详情</view>
					<view class="right_info flex_layout">
						<textarea v-model="maincontent" placeholder="请输入动态详情" />
					</view>
				</view>
				<view class="infoItem">
					<view class="left_title">动态图片</view>
		...
		<block v-if="status==1">
			<view class="sunmit_btn" @click="addSubmit">
				<button :style="{background:color}">新增动态</button>
			</view>
		</block>
		<block v-if="status==2">
			<view class="sunmit_btn" @click="editSubmit">
				<button :style="{background:color}">编辑动态</button>
			</view>
		</block>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			addSubmit(){
				...
				data['type']=this.type;
				data['title']=this.title      //动态名称
				data['maincontent']=this.maincontent       //详情
				data['picimages']=picimages     //动态图
				if(this.title==''){
					uni.showToast({
						title:'请填写动态名称',
						icon:'none'
