...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
			...
			return {
					title: title,
					path: '/pages/index/detail?company_id='+company_id+'&id='+id+'&type='+type,
					imageUrl:imgUrl
				}
		},
		mounted() {
		...
		methods: {
			addBtn(){
				uni.navigateTo({
					url:'addTrends?status=1'+'&company_id='+this.userData.company_id+'&type=cases'
				})
			},
			edit(id){
				uni.navigateTo({
					url:'addTrends?id='+id+'&status=2'+'&company_id='+this.userData.company_id+'&type=cases'
				})
			},
			companyInfoData(){
				this.reveal=false;
			...
			helpBtn(index,id,isfavor){
				...
				// this.trendsList[index].helpStatus=true
				// this.trendsList[index].helpNum++
			},
			detail(id){
				var type='news';
				var company_id=this.userData.company_id;
			...
			// NoHelpBtn(index){
			// 	this.trendsList[index].helpStatus=false
			// 	this.trendsList[index].helpNum--
			// },
		}
	}	
