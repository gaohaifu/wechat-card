	...
	<view class="content">
		...
		<view class="attestation">
			<view class="change_tab flex_layout">
				...
				<view class="tab_item" @click="titleChange(1)" :style="{color:tabIndex==1?color:''}" :class="tabIndex==1?'tab_active':''">待确认</view>
				<view class="tab_item" @click="titleChange(2)" :style="{color:tabIndex==2?color:''}" :class="tabIndex==2?'tab_active':''">已同意</view>
				<view class="tab_item" @click="titleChange(3)" :style="{color:tabIndex==3?color:''}" :class="tabIndex==3?'tab_active':''">已拒绝</view>
			</view>
			<view class="Inside_content">
				<!-- <block v-if="complete"> -->
					<block v-if="essayArry.length>0">
						<view class="Inside_fast" v-for="(item,index) in essayArry" :key="index">
							<view class="ticket_item">
								<view class="ticket_header flex_layout">
									<view class="apply_date">邀请时间：{{item.createtime}}</view>
									<block v-if="item.statusdata==1">
										<view class="status_style status_color1">待确认</view>
									</block>
									<block v-if="item.statusdata==2">
										<view class="status_style status_color2">已同意</view>
									</block>
									<block v-if="item.statusdata==3">
										<view class="status_style status_color3">已拒绝</view>
									</block>
									...
									<view class="right_content">
										<view class="ticket_message flex_layout">
											...
											<view class="return_message">
												<view>{{item.smartcardcompany.name}}</view>
											</view>
										</view>
										<view class="ticket_message flex_layout">
											<text>邀请人：</text>
								...
								<block v-if="item.statusdata==1">
								  <view class="ticket_footer flex_layout"><button @click="operateBtn(item.id,3)" class="btnStyle2":style="{color:color,border:`1px solid ${color}`}">拒绝</button><button @click="operateBtn(item.id,2)" class="btnStyle1":style="{background:color}">同意</button></view>
								</block>
							</view>
						</view>
					</block>
				...
				data => {
					if(data.code){
						...
						if (data.data == ''||data.data.length == 0) {
							...
							this.essayArry = this.essayArry.concat(arr);
						}
					}
				}
				)
			},
			operateBtn(id,statusdata){
				this.$api.messageEdit(
				{
					id:id,
...
<style>
	...
	.tab_item{padding: 30rpx; color: #333; font-size: 30rpx;}
	...
	.left_content image{display: block; width: 150rpx; height: 150rpx;}
