...
class Geetest extends Addons
{
    ...
    public function viewFilter(& $content)
    {
        ...
        if (($module == 'index' && config('fastadmin.user_register_captcha') == 'text') || ($module == 'admin' && config('fastadmin.login_captcha'))) {
            $content = preg_replace_callback('/<!--@CaptchaBegin-->([\s\S]*?)<!--@CaptchaEnd-->/i', function ($matches) {
                return '<!--@CaptchaBegin--><div style="height:44px;line-height: 44px;margin-bottom:15px;">正在加载验证码...<input type="hidden" name="captcha" /></div><!--@CaptchaEnd-->';
            }, $content);
        }
    }
}
