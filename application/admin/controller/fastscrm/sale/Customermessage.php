 ...
 * @icon fa fa-circle-o
 */
class Customermessage extends Scrmbackend
{

    /**
     * Customermessage模型对象
     * @var \app\admin\model\fastscrm\sale\Customermessage
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\fastscrm\sale\Customermessage;
    ...
    public function action($ids)
    {
        ...
        $param['task'] = 'customerSale';
        $param['ip'] = request()->ip();
        $param['id'] = $ids;
        $param['domain'] = cdnurl('', true);
        $result = $job->add($param);
        if ($result){
            Db::name('fastscrm_customer_sale')->where('id',$ids)->update(['status'=>1]);
            $this->success('群发任务已挂起');
        }else{
            $this->error('执行失败');
        }
