...
class Group extends Backend
{
    ...
    /**
     * @var \addons\myadmin\model\AuthGroup
     */
    ...
    public function _initialize()
    {
        ...
        $groupList = collection(AuthGroup::where('id', 'in', $this->childrenGroupIds)->where('company_id', 'in', [0, COMPANY_ID])->select())->toArray();
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            $parentmodel = (new \addons\myadmin\model\AuthGroup)->get($params['pid']);
            if (!$parentmodel) {
    ...
    public function edit($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    $row->save($params);
                    $children_auth_groups = (new \addons\myadmin\model\AuthGroup)->all(['id' => ['in', implode(',', (Tree::instance()->getChildrenIds($row->id)))]]);
                    $childparams = [];
                    ...
                    (new \addons\myadmin\model\AuthGroup)->saveAll($childparams);
                    Db::commit();
                    $this->success();
                } catch (Exception $e) {
                    Db::rollback();
    ...
    public function del($ids = "")
    {
        ...
        if ($ids) {
            ...
            // 循环判断每一个组别是否可删除
            $grouplist = $this->model->where('id', 'in', $ids)->where('company_id', COMPANY_ID)->select();
            if(!$grouplist){
                $this->error(__('No rows were deleted'));
            }
            $groupaccessmodel = new \addons\myadmin\model\AuthGroupAccess;
            foreach ($grouplist as $k => $v) {
                // 当前组别下有管理员
            ...
            $count = $this->model->where('id', 'in', $ids)->where('company_id', COMPANY_ID)->delete();
            if ($count) {
                $this->success();
            }
        }
    ...
    public function roletree()
    {
        ...
        $model = new \addons\myadmin\model\AuthGroup;
        ...
        if (($pid || $parentGroupModel) && (!$id || $currentGroupModel)) {
            $id = $id ? $id : null;
            $ruleList = collection((new \addons\myadmin\model\AuthRule)->order('weigh', 'desc')->order('id', 'asc')->select())->toArray();
            ...
            //角色组列表
            $groupTree->init(collection((new \addons\myadmin\model\AuthGroup)->where('id', 'in', $this->childrenGroupIds)->select())->toArray());
            ...
            if (!$id || !in_array($pid, $this->childrenGroupIds) || !in_array($pid, $groupTree->getChildrenIds($id, true))) {
                ...
                $nodeList = [];
                $RuleIds = Company::getRules(COMPANY_ID);
                foreach ($parentRuleList as $k => $v) {
                    if (!in_array($v['id'], $RuleIds)) {
                        unset($ruleList[$k]);
                        continue;
                    }
