            ...
            if ($params) {
                ...
                try {
                    ...
                    foreach ($group as $value) {
                        $dataset[] = ['uid' => $this->model->id, 'group_id' => $value, 'company_id' => COMPANY_ID];
                    }
            ...
            if ($params) {
                ...
                try {
                    ...
                    $adminValidate->rule([
                        ...
                        'email'    => 'email|unique:MyadminAdmin,email,' . $row->id,
                        'password' => 'regex:\S{32}',
                    ]);
                    ...
                    // 先移除所有权限
                    (new AuthGroupAccess)->where('uid', $row->id)->delete();
                    ...
                    $dataset = [];
                    foreach ($group as $value) {
                        $dataset[] = ['uid' => $row->id, 'group_id' => $value, 'company_id' => COMPANY_ID];
                    }
                    (new AuthGroupAccess)->saveAll($dataset);
                    Db::commit();
                } catch (\Exception $e) {
    ...
    public function del($ids = "")
    {
        ...
        if ($ids) {
            ...
            $adminList = $this->model->where('id', 'in', $ids)->where('id', 'in', function ($query) use ($childrenGroupIds) {
                $query->name("myadmin_auth_group_access")->where('group_id', 'in', $childrenGroupIds)->field('uid');
            })->where('is_founder', 0)->select();
            if ($adminList) {
                $deleteIds = [];
